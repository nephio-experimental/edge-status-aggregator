...
IMG ?= edge-status-aggregator:latest
...
.PHONY: test
test: manifests generate-controller generate-mock fmt vet envtest ## Run tests.
	KUBEBUILDER_ASSETS="$(shell $(ENVTEST) use $(ENVTEST_K8S_VERSION) -p path)" go test ./... -coverprofile cover.out

##@ Build

.PHONY: build
build: generate-controller fmt vet ## Build manager binary.
	go build -o bin/manager main.go

.PHONY: run
run: manifests generate-controller fmt vet ## Run a controller from your host.
	go run ./main.go
...
deploy: manifests kustomize ## Deploy controller to the K8s cluster specified in ~/.kube/config.
	...
	$(KUSTOMIZE) build config/default | cat > config/deployment/deployment.yaml
